...
class SAM3Annotator(QMainWindow):
    ...
    def _setup_ui(self):
        ...
        # Right panel - Controls (with scroll area for small screens)
        ...
        right_layout.setContentsMargins(10, 10, 20, 10)  # 오른쪽 여백 추가 (스크롤바 공간)
        ...
        main_splitter.setSizes([1100, 320])
    ...
    def _open_image(self, file_path: str):
        try:
            ...
            pil_image_raw = Image.open(file_path).convert("RGB")
            self.pil_image = ImageOps.exif_transpose(pil_image_raw)
            if self.pil_image is None:
                self.pil_image = pil_image_raw
